  ...
  group('LoginCubit', () {
    ...
    group('logInWithTwitter', () {
      blocTest<LoginCubit, LoginState>(
        ...
        build: () => LoginCubit(authenticationRepository),
        act: (cubit) => cubit.logInWithTwitter(),
        verify: (_) {
          verify(() => authenticationRepository.logInWithTwitter()).called(1);
        },
      );
      ...
      blocTest<LoginCubit, LoginState>(
        'emits [submissionInProgress, submissionFailure] '
        'when logInWithTwitter fails due to LogInWithTwitterFailure',
        setUp: () {
          when(
            () => authenticationRepository.logInWithTwitter(),
          ).thenThrow(LogInWithTwitterFailure('oops'));
        },
        build: () => LoginCubit(authenticationRepository),
        act: (cubit) => cubit.logInWithTwitter(),
        expect: () => const <LoginState>[
      ...
      blocTest<LoginCubit, LoginState>(
        'emits [submissionInProgress, submissionFailure] '
        'when logInWithTwitter fails due to generic exception',
        setUp: () {
          when(
            () => authenticationRepository.logInWithTwitter(),
          ).thenThrow(Exception('oops'));
